...
export default function App() {
  ...
  useEffect(() => {
    ...
    const handleMouseOver = (e) => {
      const isHoverable = e.target.closest(
        "button, a, .hoverable, .btn-brutal, .meteor-card, .card-brutal",
      );
      if (isHoverable) {
    ...
    const handleMouseClick = (e) => {
      ...
      setTimeout(() => {
        burst.style.transition =
          "opacity 0.3s, transform 0.3s cubic-bezier(0.6, -0.28, 0.735, 0.045)";
  ...
  if (answers && view !== "catalog" && !query.trim()) {
  ...
  const toggleCompare = (id) => {
    setCompareIds((c) => {
      ...
      if (c.length >= 4) {
        toast.show("You can compare up to 4 laptops");
        return c;
      }
      return [...c, id];
    });
  ...
  const openScoreFor = (l) =>
    scored.find((s) => s.l.id === l.id)?.score ?? null;
  ...
  return (
    <div className="app-root min-h-screen relative overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col min-h-screen">
          ...
          {view === "wishlist" && (
            ...
            >
              <ResultsView
                isWishlist={true}
                answers={answers}
                filtered={filtered.filter((s) => wishlist.includes(s.l.id))}
                query={query}
                setQuery={setQuery}
                wishlist={wishlist}
                toggleWish={toggleWish}
        ...
        {toast.msg && (
          <div
            className="fixed top-20 left-1/2 -translate-x-1/2 z-50 text-sm px-4 py-2 mono"
        ...
        <DetailModal
          l={openLaptop}
          score={openLaptop ? openScoreFor(openLaptop) : 0}
        ...
        >
          <div className="max-w-6xl mx-auto px-5 py-8 text-xs font-semibold uppercase flex flex-wrap gap-x-6 gap-y-2 mono text-gray-600">
            ...
            <span
              ...
              className="hover:text-black cursor-pointer hoverable"
            >
              Buying Guide
            </span>
            <span
