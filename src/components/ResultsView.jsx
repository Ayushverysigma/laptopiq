      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 mb-12">
        <div>
          ...
          <p className="mono font-bold text-[var(--ink-soft)] uppercase tracking-wider">
            {isWishlist
              ...
              : answers
                ? `Ranked for ${answers.studentType || "you"}`
        ...
        <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto items-center">
          {!answers && (
            <button
              onClick={goToQuiz}
              className="btn-brutal btn-brutal-primary whitespace-nowrap px-6 py-3 hoverable"
            >
              Take the quiz
            </button>
          )}
          <div className="relative w-full sm:w-80 group">
            <Search
              size={18}
              strokeWidth={3}
              className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--ink-soft)] group-focus-within:text-[var(--blue)] transition-colors z-10"
            />
            <input
              value={query}
              onChange={(e) => {
      ...
      {filtered.length === 0 && (
        <div className="text-center py-24 border-[3px] border-[var(--ink)] bg-[var(--paper-2)] shadow-[8px_8px_0_var(--ink)] rounded-2xl mt-8">
          <Search
            size={48}
            strokeWidth={2.5}
            className="mx-auto text-[var(--ink-soft)] mb-6"
          />
          <p className="display text-4xl text-[var(--ink)] uppercase mb-2">
            {isWishlist && !query
              ? "Your wishlist is empty"
              : `No laptops match "${query}"`}
          </p>
          <p className="mono font-bold text-[var(--ink-soft)] uppercase tracking-wider">
            {isWishlist && !query
              ? "Save laptops to compare them later."
              : "Try searching for a different processor or brand."}
          </p>
        </div>
      )}
