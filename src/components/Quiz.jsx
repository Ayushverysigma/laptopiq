...
export default function Quiz({ onComplete, onExit }) {
  ...
  const slideVariants = {
    ...
    enter: { opacity: 1, x: 0, scale: 1, transition: { duration: 0.5, ease: [0.22, 1, 0.36, 1] } },
    exit: { opacity: 0, x: -60, scale: 0.95, transition: { duration: 0.4, ease: [0.22, 1, 0.36, 1] } }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto relative z-10 min-h-[75vh] flex flex-col justify-center py-10">
      <div className="absolute top-0 left-0 w-full flex items-center justify-between mb-12">
        <button onClick={onExit} className="text-sm font-bold mono uppercase tracking-widest flex items-center gap-2 text-[var(--ink-soft)] hover:text-[var(--ink)] transition-colors duration-300">
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mb-12">
              {q.options.map((opt, i) => {
                const active = q.multi ? (answers[q.id] || []).includes(opt) : answers[q.id] === opt;
                return (
                  <button 
                    key={opt}
                    onClick={() => select(opt)} 
                  ...
                  >
                    <span className="flex items-center justify-between gap-2 relative z-10 whitespace-normal break-words h-full">
                      <span>{opt}</span>
                      <span className={`rounded-full p-1 border-2 border-[var(--ink)] shrink-0 transition-opacity duration-200 ${active ? 'opacity-100 text-[var(--yellow)] bg-[var(--ink)]' : 'opacity-0 pointer-events-none'}`}>
      ...
      <div className="flex justify-between items-center mt-auto pt-8 border-t-[3px] border-[var(--ink)]">
        <button
          disabled={history.length === 1} onClick={prev}
        ...
        <button disabled={!canNext} onClick={next} className="btn-brutal btn-brutal-primary hoverable disabled:opacity-50 disabled:shadow-[5px_5px_0_var(--ink)] disabled:translate-x-0 disabled:translate-y-0 disabled:hover:bg-[var(--red)] flex items-center gap-2">
          {q.next(answers) === null ? "Generate Matches" : "Continue"} <ChevronRight size={20} strokeWidth={3} />
        </button>
      </div>
    </div>
  );
