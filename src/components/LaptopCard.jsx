import { Heart, Scale, Cpu, MonitorSmartphone, Battery, Weight, ExternalLink, HelpCircle } from "lucide-react";
...
export default function LaptopCard({ l, rank, score, misses, wished, onWish, compared, onCompare, canCompare, onOpen, insight, customStamp, isTopPick }) {
  ...
  return (
    ...
    >
      {/* Top Banner Ribbon */}
      {showTopBadge && (
        <div className={`absolute top-[-3px] right-6 border-x-[3px] border-b-[3px] border-[var(--ink)] px-4 py-1.5 rounded-b-xl display tracking-widest uppercase z-10 shadow-[2px_2px_0_var(--ink)] ${badgeClass}`}>
          {label}
        </div>
      )}
      ...
      <div className="flex items-start justify-between gap-4 mt-8 mb-4">
        <div>
          <h3 className="display text-3xl leading-none mb-2 text-[var(--ink)] group-hover:text-[var(--red)] transition-colors uppercase tracking-wide">{l.name}</h3>
          <p className="mono text-xs font-bold text-[var(--ink-soft)] uppercase tracking-wider">{l.brand} · {l.os}</p>
        </div>
        {showTopBadge && (
      ...
      <div className="flex items-baseline gap-3 mono font-bold mb-6 pb-4 border-b-[3px] border-[var(--ink)]">
        ...
        <span className="text-sm line-through text-[var(--ink-soft)]">{inr(l.mrp)}</span>
        {discount > 0 && <span className="text-[10px] px-2 py-1 rounded-full bg-[var(--red)] text-white border-2 border-[var(--ink)]">{discount}% OFF</span>}
      </div>

      {/* Specs Chips */}
      <div className="flex flex-wrap gap-2 mb-6">
        <span className="mono text-[9px] tracking-widest uppercase font-bold border-2 border-[var(--ink)] rounded-full px-3 py-1 bg-[var(--paper-2)] text-[var(--ink)] flex items-center gap-1"><Cpu size={12}/> {l.processor}</span>
        <span className="mono text-[9px] tracking-widest uppercase font-bold border-2 border-[var(--ink)] rounded-full px-3 py-1 bg-[var(--paper-2)] text-[var(--ink)] flex items-center gap-1"><MonitorSmartphone size={12}/> {l.display}</span>
        <span className="mono text-[9px] tracking-widest uppercase font-bold border-2 border-[var(--ink)] rounded-full px-3 py-1 bg-[var(--paper-2)] text-[var(--ink)] flex items-center gap-1"><Battery size={12}/> {l.battery}H</span>
        <span className="mono text-[9px] tracking-widest uppercase font-bold border-2 border-[var(--ink)] rounded-full px-3 py-1 bg-[var(--paper-2)] text-[var(--ink)] flex items-center gap-1"><Weight size={12}/> {l.weight}KG</span>
      </div>
      ...
      {score !== null && misses && misses.length > 0 && (
        <div className="text-xs leading-relaxed p-3 mb-6 font-bold mono border-[3px] border-[var(--ink)] bg-[#FFF0E5] text-orange-700 shadow-[3px_3px_0_var(--ink)]">
          ...
          <ul className="list-square pl-4 space-y-1">
            {misses.map((m, idx) => <li key={idx}>{m}</li>)}
          </ul>
        </div>
      )}
      ...
      <div className="mt-auto flex items-center gap-3">
        <button 
          onClick={(e) => { e.stopPropagation(); onWish(l.id); }} 
        ...
        >
          <Heart size={20} strokeWidth={2.5} fill={wished ? "currentColor" : "none"} /> {wished ? "Saved" : "Save"}
        </button>
        <button 
          onClick={(e) => { e.stopPropagation(); onCompare(l.id); }} 
