...
export function scoreLaptop(l, a) {
  ...
  if (isNoBudget) {
    if (l.price < 100000) {
      misses.push(`Not a premium high-end tier (₹${l.price.toLocaleString('en-IN')})`);
      score -= 15;
    ...
    if (l.price > hi + buffer) {
      ...
      const overage = l.price - hi;
      misses.push(`Slightly over budget by ₹${overage.toLocaleString('en-IN')}`);
  ...
  if (a.os && a.os !== "Doesn't Matter") {
    if (a.os === "macOS" && l.os !== "macOS") {
      misses.push(`Requires macOS (This is ${l.os})`);
    } else if (a.os === "Windows" && l.os !== "Windows") {
  ...
  if (a.gpuPref === "Yes, absolutely (Heavy gaming/CAD)") {
    ...
    const hasDedicated = gpuLower.includes("rtx") || gpuLower.includes("rx") || gpuLower.includes("dedicated") || gpuLower.includes("apple");
    if (!hasDedicated) {
    ...
    const hasIntegrated = l.gpu.toLowerCase().includes("integrated") || l.gpu.toLowerCase().includes("iris") || l.gpu.toLowerCase().includes("adreno");
  ...
  const heavyWorkloads = ["Gaming", "Video Editing", "3D Rendering", "AI/ML", "CAD (3D Modeling)", "Graphic Design"];
...
export function getYearFromProcessor(processor = "", name = "") {
  ...
  if (p.includes("14th gen") || p.includes("core ultra") || p.includes("ryzen 8") || p.includes("ryzen ai") || p.includes("m3 ") || p.includes("m4 ") || p.includes("snapdragon x")) return 2024;
  if (p.includes("13th gen") || p.includes("ryzen 7") || p.includes("m2 pro") || p.includes("m2 max")) return 2023;
  if (p.includes("12th gen") || p.includes("ryzen 6") || p.includes("m2 ")) return 2022;
  if (p.includes("11th gen") || p.includes("10th gen") || p.includes("ryzen 5") || p.includes("ryzen 4") || p.includes("ryzen 3") || p.includes("m1 ")) return 2020;
...
export const TONE_VAR = { pass: "var(--green)", good: "var(--blue)", fair: "var(--yellow)", low: "var(--red)" };
